...
T5-base and T5-large had similar agreement with us human annotators, and their generated intents are rather similar in nature. 
The main difference we saw between these 2 models and our human annotations is that humans tend to not use word-for-word reiterations of the prompt, but rather summarize the intent by rephrasing some of the terms with aims to capture the intent in a concise way. On the other hand, the prompts seen by T5-base and T5-large were less condense, had less rephrasing, and tended to stick to exact terminology used in the prompt. 

However, T5-small seems to disagree with human annotators the most out of all three T5 models. Upon closer inspection of the generated intents, it seems that its main difference and struggle is the level of detail and scope ofthe intent - some generated intents are very vague and do not capture the nuance of the situation, or too detailed such that it almost captures the prompt itself word-for-word, which is quite different from human annotations as previously mentioned. .
...
From the results on the `WildGuard` test set and the `AmazonScience/FalseReject` dataset we do observe that the similarity between annotator and model prediction is quite similar across these datasets. Interestingly, the model-annotator cosine similarity for the `AmazonScience/FalseReject` dataset is higher for the T5-base and T5-large models compared to the `WildGuard` test set. This shows good generalizability as it performs just as well on a new dataset.